...
public class CocktailDbActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void loadDataInListView() {
        ...
        myAdapter = new MyAdapter(this,arrayList,countryFlags);
    ...
    private final AdapterView.OnItemClickListener onClickListener = new AdapterView.OnItemClickListener(){
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id){
            ...
            nowData[0] = ((Cocktail)myAdapter.getItem(position)).getName();
            nowData[1] = Integer.toString(((Cocktail)myAdapter.getItem(position)).getSugar());
            nowData[2]=Integer.toString(((Cocktail)myAdapter.getItem(position)).getAlcohol());
            nowData[3]=Integer.toString(((Cocktail)myAdapter.getItem(position)).getBody());
            nowData[4]=Integer.toString(((Cocktail)myAdapter.getItem(position)).getUnique_());
            nowData[5]=((Cocktail)myAdapter.getItem(position)).getBase();
            nowData[6]=Integer.toString(((Cocktail)myAdapter.getItem(position)).getId());
    ...
    private final AdapterView.OnItemLongClickListener longClickListener = new AdapterView.OnItemLongClickListener(){
        ...
        public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            /*
            Log.d("Long Click nowdata 0: ",nowData[0]);
            Log.d("Long Click nowdata 1: ",nowData[1]);
            Log.d("Long Click nowdata 2: ",nowData[2]);
            Log.d("Long Click nowdata 3: ",nowData[3]);
            Log.d("Long Click nowdata 4: ",nowData[4]);
            Log.d("Long Click nowdata 5: ",nowData[5]);
            Log.d("Long Click nowdata 6: ",nowData[6]);
             */
            String viewData = nowData[0]+", "+nowData[1]+", "+nowData[2]+", "+nowData[3]+", "+nowData[4]+", "+nowData[5];
            AlertDialog.Builder dialog = new AlertDialog.Builder(CocktailDbActivity.this);
            dialog.setTitle("데이터 이동")
                    .setMessage("해당 데이터를 이동 하시겠습니까?"+"\n"+viewData)
                    .setPositiveButton("네",(dialog12, which)-> {
                        Toast.makeText(CocktailDbActivity.this, "데이터를 이동했습니다.", Toast.LENGTH_SHORT).show();
                        mDbOpenHelper.insertData(nowData, Long.valueOf(nowData[6]));
                    })
                    .setNegativeButton("아니오",(dialog1,which)->{
                        Toast.makeText(CocktailDbActivity.this,"이동을 취소했습니다.",Toast.LENGTH_SHORT).show();
                    })
                    .create()
                    .show();
            return false;
